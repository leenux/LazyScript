...
function lazyScript.bitParsers.pop(bit, actions, masks)
	if (not lazyScript.rebit(bit, "^pop=(.+)$")) then
		return false
	end
	
	local message = lazyScript.match1
	
	local key = "pop="..message
	if (not lazyScript.otherActions[key]) then
		lazyScript.otherActions[key] = lazyScript.PopAction:New("pop", message)
	end
	table.insert(actions, lazyScript.otherActions[key])
	return true
end
...
function lazyScript.masks.Reloading()
	if (not IsAddOnLoaded("Quiver")) then
		lazyScript.p("Need Quiver addons")
		return false
	end	
	return not Quiver.PredMidShot()
end

function lazyScript.bitParsers.ifReloading(bit, actions, masks)
	if (not lazyScript.rebit(bit, "^if(Not)?Reloading$")) then
		return false
	end
	...
	table.insert(masks, lazyScript.masks.HaveTarget)
	local negate = lazyScript.negate1()
	table.insert(masks, lazyScript.negWrapper(lazyScript.masks.Reloading, negate))
